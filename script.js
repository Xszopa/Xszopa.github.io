...
class SnakeGame {
    constructor(canvas) {
        ...
        this.gridSize = 20; // Rozmiar komórek
    ...
    placeApple() {
        this.apple.x = Math.floor(Math.random() * (this.canvas.width / this.gridSize));
        this.apple.y = Math.floor(Math.random() * (this.canvas.height / this.gridSize));
    }
    ...
    gameLoop() {
        if (this.running) {
            this.update();
            this.draw();
            setTimeout(() => this.gameLoop(), 100); // Ustawienie szybkości gry
        }
    }
}
